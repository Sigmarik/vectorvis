...
void Widget::onEvent(const plug::Event& event, plug::EHC& context) {
    switch (event.getType()) {
        ...
        case plug::EventType::KeyboardPressed: {
            onKeyboardPressed((const plug::KeyboardPressedEvent&)event,
                              context);
        } break;
        case plug::EventType::KeyboardReleased: {
            onKeyboardReleased((const plug::KeyboardReleasedEvent&)event,
                               context);
        } break;
...
void Widget::onMouseMove(const plug::MouseMoveEvent& event,
                         plug::EHC& context) {
    context.overlapped = context.overlapped || covers(context.stack, event.pos);
}

void Widget::onMousePressed(const plug::MousePressedEvent& event,
                            plug::EHC& context) {
    bool cover = covers(context.stack, event.pos);

    context.overlapped = context.overlapped || cover;
    context.stopped = context.stopped || cover;
}

void Widget::onMouseReleased(const plug::MouseReleasedEvent& event,
                             plug::EHC& context) {
    context.overlapped = context.overlapped || covers(context.stack, event.pos);
}
...
Vec2d Widget::getCorner(Corner corner,
                        const plug::TransformStack& stack) const {
    return stack.top().apply(getAbsCorner(corner));
}

Vec2d Widget::getAbsCorner(Corner corner) const {
    Vec2d direction((corner & 1) ? 0.5 : -0.5, (corner & 2) ? -0.5 : 0.5);
    return box_->getPosition() + box_->getSize() * direction;
}
