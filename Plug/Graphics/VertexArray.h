...
class Vertex {
   public:
     ...
     *
     */
    Vec2d tex_coords = Vec2d(0.0, 0.0);
     ...
     *
     */
    Color color = plug::Color(255, 255, 255, 255);
};
...
inline VertexArray::VertexArray(PrimitiveType type, size_t size)
    : m_type(type), m_data(new Vertex[size]), m_size(size), m_capacity(size) {}

inline VertexArray::VertexArray(const VertexArray &other)
    : m_type(other.m_type),
      m_data(new Vertex[other.m_capacity]),
      m_size(other.m_size),
...
inline void VertexArray::appendVertex(const Vertex &vertex) {
    if (m_size == m_capacity) resize(m_size * 2);
